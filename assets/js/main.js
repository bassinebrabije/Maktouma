document.addEventListener("DOMContentLoaded", (event) => {
    ...
    // HEADER NAV IN MOBILE & SIDEBAR
    // Separate sidebar open/close from header-nav existence so sidebar works even
    // when the main header nav is absent (some pages comment it out).
    ...
    // Sidebar opener/closer: attach listeners only when elements exist
    if (ulSidebar && ulSidebarOpener) {
        ulSidebarOpener.addEventListener("click", () => {
            ulSidebar.classList.add("active");
        });
    }

    if (ulSidebar && ulSidebarCloser) {
        ulSidebarCloser.addEventListener("click", () => {
            ulSidebar.classList.remove("active");
        });
    }

    // Move header nav into sidebar header on mobile (if the expected elements exist)
    ...
    if (ulMobileMenuContent && ulHeaderNavMobileWrapper && ulHeaderNavOgWrapper) {
        updateMenuPosition();
        window.addEventListener("resize", updateMenuPosition);
    }

    // menu dropdown/submenu in mobile (only if header nav exists)
    if (ulHeaderNav) {
        const ulHeaderNavMobileItems = ulHeaderNav.querySelectorAll(".has-sub-menu");
